...
ace.handle_side_menu = function (a) {
    ...
    a(".nav-list").on(ace.click_event, function (h) {
        ...
        if (!g.hasClass("dropdown-toggle")) {
            ...
            } else if (ace.click_event == "click"){
                //同级清除
                siblings = a(g.get(0).parentNode).siblings();
                ace.show_all(siblings,a);
                //如果点击的是首页，则只清除同级的其他父级
                if (a(this).children().first().find("a").attr("id") != a(g.get(0).parentNode).find("a").attr("id")) {
                    //父级
                    parents = a(g.get(0).parentNode).parents("li");
                    for (var i = 0; i < parents.length; i ++) {
                        //清除父级其他节点
                        otherParents = a(parents[i]).siblings();
                        ace.show_all(otherParents,a);

                        parent = parents[i];
                        // console.log(">>> parent : " + parent);
                        //给父节点添加class
                        a(parents[i]).addClass("active");
                    }
                }
                //给点击节点添加class
                a(g.get(0).parentNode).addClass("active");
            }
            if (b) {
...
ace.show_all = function (array,a) {
    //获取根节点
    for(var i = 0; i < array.length; i ++) {
        item = array[i];
        //移除class
        a(item).removeClass();
        a(item).find("ul").removeAttr("style");
        childrens = a(item).find("li");
        //console.log("item : " + item + " childrens.length :" + childrens.length);
        ace.show_all(childrens,a);
    }
};
...
ace.show_msg = function (msg) {
    jQuery("#alert-info").html("<i class='icon-hand-right'></i> " + msg +"<button class='close' data-dismiss='alert'><i class='icon-remove'></i></button>");
};
